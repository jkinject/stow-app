...
export const KO = {
  ...
  item: {
    ...
    deleteItem: '이 물건 지우기',
    /**
     * 사용중 상태 (2026-09-17 사용자 요청).
     *
     * 멀티탭처럼 가끔 꺼내 쓰고 **다시 그 자리에** 두는 물건 — 위치를 옮기는 게 아니라
     * "지금은 제자리에 없다" 는 임시 표시다. 그래서 문구도 이동이 아니라 꺼냄·돌려놓음이다.
     */
    inUse: {
      label: '상태',
      stored: '제자리에 보관 중',
      ...
      /** 격자 카드의 뱃지 */
      badge: '사용중',
      /** "팀 · 3일 전부터" — 누가 가지고 있는지가 곧 어디 있는지다 */
      since: (who: string, when: string) => `${who} · ${when}부터`,
      takeOut: '꺼내 쓰기',
      putBack: '제자리에 뒀어요',
      tookOut: '사용중으로 표시했습니다. 다 쓰면 제자리에 두고 알려 주세요.',
      putBackDone: '제자리에 뒀습니다.',
      failed: '상태를 바꾸지 못했습니다',
    },
  ...
  locSheet: {
    ...
    stage: '담기',
    /**
     * ⚠ '고른 곳 · N곳' 이었다. "고른 곳" 이 어색하다는 지적(2026-09-02).
     *   이 줄 아래에는 **직접 입력한 것만** 보이므로(제안 칩은 위에서 켜진 채로
     *   보인다) 이름도 그대로 '직접 입력' 이라고 말하는 편이 정확하다.
     */
    picked: (n: number) => `직접 입력 · ${n}곳`,
    /** 이미 있는 이름을 또 적었을 때. 조용히 삼키면 왜 안 담기는지 알 수 없다 */
    alreadyExists: (name: string) => `'${name}' 장소는 이미 있습니다.`,
    done: '완료',
    /** 담아 둔 게 있을 때는 완료가 곧 저장이다 — 몇 개가 저장되는지 밝힌다 */
    saveN: (n: number) => `${n}곳 추가`,
    saving: '추가하는 중…',
    addFailed: (names: string) => `추가하지 못했습니다: ${names}`,
    discardTitle: '담아 둔 장소를 버릴까요?',
