...
export type SearchRow = {
  ...
  /** 꺼내 쓰는 중이면 그 시각 (2026-09-17). 격자 배지 */
...
export async function fetchAllItems(householdId: string): Promise<SearchRow[]> {
  const { data, error } = await supabase
    .from('items')
    .select('id, name, quantity, unit, thumb_path, expires_on, in_use_since, location_id, container_id, updated_at, created_at, category:categories!items_category_id_fkey(name, color)')
